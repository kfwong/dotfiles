require '/home/kfwong/.config/obmenu-generator/config.pl';
...
our $SCHEMA = [
	...
	{raw => q(<menu id="Screenshot" label="Take Screenshot">)},
		{raw => q(<item label="Now"><action name="Execute"><execute>scrot '%Y-%m-%d--%s_$wx$h_scrot.png' -e 'mv $f ~/Pictures/Screenshots; notify-send -u low "Screenshot" "Saved."'</execute></action></item>)},
		{raw => q(<item label="In 10 Seconds..."><action name="Execute"><execute>scrot -d 10 '%Y-%m-%d--%s_$wx$h_scrot.png' -e 'mv $f ~/Pictures/Screenshots; notify-send -u low "Delayed Screenshot" "Saved."'</execute></action></item>)},
		{raw => q(<item label="Selected Area... click &amp; drag mouse"><action name="Execute"><execute>scrot -s '%Y-%m-%d--%s_$wx$h_scrot.png' -e 'mv $f ~/Pictures/Screenshots; notify-send -u low "Selective Screenshot" "Saved."'</execute></action></item>)},
	{raw => q(</menu>)},
#		{item => ['obkey',   'Edit Key Bindings',   undef]},
	...
	#{exit => ['Exit', 'exit']},
	{item => ['slock', 'Lock', undef]},
	{item => ['systemctl suspend', 'Suspend', undef]},
